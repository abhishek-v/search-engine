...
class PageRank:
    def __init__(self):
        '''WARNING: EXPLICIT MODIFICATION NEEDED FOR FOLLOWING VARIABLE and line 25'''
        data_file = open("crawl_2","rb")
        self.link_reference = pickle.load(data_file)
        self.reverse_link_reference = pickle.load(data_file)
        data_file.close()
        data_file = open("preprocess_2","rb")
    ...
    def pagerank(self):
        ...
        for index,value in self.nodes_outlink.items():
            ...
            print(value)
            print(index)
            print(self.link_reference[index]+" is empty")
        ...
        for i in range(iterations):
            ...
            for index in self.link_reference.keys(): #loop through each node and calculate its page rank
                #page rank formula:
                temp_score[index] = (self.dampFactor * self.formula(index)) + ((1-self.dampFactor)/self.nodeCount)
    ...
    def formula(self, curNode):
        ...
        for neighbour,value in self.nodes_inlink[curNode].items():
            ...
            denominator = len(self.nodes_outlink[neighbour].keys())
            if(denominator ==0):
                print("yes")
...
if(__name__=="__main__"):
    ...
    print(pr.link_reference[page_order[1][0]])
    exit()
